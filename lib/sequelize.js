...
Sequelize.prototype.getCLOB = function(results, listCLOB, done) {
  ...
  async.eachLimit(results, 5, function(item, nextItem) {

    async.each(listCLOB, function(key, nextKey) {
...
Sequelize.prototype.insertCLOB = function(model, data, ClobFields, done) {
  ...
  model
    ...
    .then(function(result) {
      ...
      table.push(model.options.tableName);
      table = table.join('.');
      ...
      _this.connectionManager.connect(_this.config).then(function(connection) {
        ...
        connection.execute(
          ...
          function(err, result) {
            ...
            async.forEachOf(ClobData, function(value, key, nextKey) {
